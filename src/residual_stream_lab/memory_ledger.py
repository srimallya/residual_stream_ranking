...
class MemoryLedger:
    ...
    def register_event(
        ...
        *,
        ...
        if behavior_helped:
            memory_object.downstream_utility += 1.0
    ...
    def candidate_rows(self, *, limit: int = 5) -> list[dict[str, object]]:
        ...
        for memory_object in self.objects.values():
            if memory_object.pinned:
                continue
            if memory_object.downstream_utility > 0:
                continue
            if memory_object.replay_usage_count == 0:
                continue
            suggested_tier = "archived" if memory_object.retrieved_count >= 2 else "cold"
            candidates.append(
                {
